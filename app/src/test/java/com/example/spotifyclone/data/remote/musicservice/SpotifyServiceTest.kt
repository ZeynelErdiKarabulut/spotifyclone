package com.example.spotifyclone.data.remote.musicservice

import com.example.spotifyclone.data.encoder.TestBase64Encoder
import com.example.spotifyclone.data.remote.response.AlbumMetadataResponse
import com.example.spotifyclone.data.remote.response.TracksWithAlbumMetadataListResponse
...
class SpotifyServiceTest {
    ...
    private val tokenRepository = SpotifyTokenRepository(
        Retrofit.Builder().baseUrl(SpotifyBaseUrls.AUTHENTICATION_URL)
            .addConverterFactory(defaultspotifycloneJacksonConverterFactory).build()
